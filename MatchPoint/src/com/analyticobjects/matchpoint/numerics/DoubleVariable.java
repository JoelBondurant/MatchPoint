...
public class DoubleVariable extends Variable
{
     ...
     * 		  &lt; 0 || index &gt;= size())</tt>.
     */
    public boolean remove(int[] deletedRows)
    {
        //I assume that the deletedRows array is sorted in ascending order.
        //Arrays.sort(deletedRows);
        if(deletedRows == null){return false;}
        if(deletedRows[0] >= size){return false;}
        dirty = true;
        int sizeToGCAt = 100000;
        int maxOverhead = 10000;
        int lastDeletedRowIndex = 0;
        for(int i=0; i<deletedRows.length; i++)//calculate the last row that should be deleted.
        {
        ...
        int chunkEnd = -1;
        int chunkSize = 0;
        int destinationIndexProgress = 0;
        if(deletedRows[0]!=0)//move the first data chunk if at beginning.
        {
            chunkStart = 0;
            chunkEnd = deletedRows[0] - 1;
            chunkSize = chunkEnd - chunkStart + 1;
            System.arraycopy(elementData, chunkStart, newElementData, destinationIndexProgress, chunkSize);
            //System.out.print("(chunkStart, chunkEnd, destinationIndexProgress, chunkSize) = ");
            //System.out.println("("+chunkStart+", "+chunkEnd+", "+destinationIndexProgress+", "+chunkSize+")");
            destinationIndexProgress += chunkSize;
        }
        int deletedRowIndex = 0;
        while(deletedRowIndex <= lastDeletedRowIndex)//loop over chunks;
        {
            while(deletedRowIndex < lastDeletedRowIndex && deletedRows[deletedRowIndex] + 1 == deletedRows[deletedRowIndex + 1])//loop to find chunkStart;
            {
                ++deletedRowIndex;
            }
            chunkStart = deletedRows[deletedRowIndex] + 1;
            if(chunkStart >= size){break;}
            chunkEnd = (deletedRowIndex == lastDeletedRowIndex)?(size - 1):(deletedRows[deletedRowIndex + 1] - 1);
            chunkSize = chunkEnd - chunkStart + 1;
            System.arraycopy(elementData, chunkStart, newElementData, destinationIndexProgress, chunkSize);
            //System.out.print("(chunkStart, chunkEnd, destinationIndexProgress, chunkSize) = ");
            //System.out.println("("+chunkStart+", "+chunkEnd+", "+destinationIndexProgress+", "+chunkSize+")");
        ...
        //end copy/transfer array data here.
