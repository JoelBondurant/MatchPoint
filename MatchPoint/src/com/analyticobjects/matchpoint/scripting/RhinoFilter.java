...
public class RhinoFilter
{
    ...
    private class Constant
    {
        ...
        Constant(String name, boolean justCreated, int location)
        {
            ...
            String resetRegEx;
            //the following operators can modify a variable (not in a string though ("x = " + x))
            //this will bomb str = " x = 9 ";, workaround => str = " x " + "= 9 ";
            //(=, +=, -=, *=, /=, >>=, <<=, >>>=, &=, |=, ^=, ++x, x++, --x, x--)
            resetRegEx = "((^| |;)" + name + " *(\\+|-|\\*|/|\\||>>|<<|>>>|<<<|&|^)?=[^=])";
            resetRegEx += "|(\\+\\+ *" + name + ")";
            resetRegEx += "|(" + name + " *\\+\\+)";
            resetRegEx += "|(-- *" + name + ")";
            resetRegEx += "|(" + name + " *--)";
            resetPattern = Pattern.compile(resetRegEx);
        }
    }
